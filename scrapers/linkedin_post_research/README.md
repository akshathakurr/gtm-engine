...
| `date_filter` | string \| null | null | Optional time window — one of `1h`, `24h`, `week`, `month` (leave null for no filter) |
...
- `max_posts` is enforced server-side via the actor's `maxPosts` input (no pagination loop).
