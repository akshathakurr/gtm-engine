...
### 23. Onboarding is clone + Claude-does-everything — no installer, no npm package

The front door is a plain `git clone … && cd gtm-engine && claude`, then the user says "help me get started" and **Claude Code does 100% of setup conversationally**: copies `.env.example` → `.env`, guides keys, copies `context.md.example` → `context.md`, runs the business interview, shows the workflows. There is deliberately **no installer, no `npx create-gtm-engine`, no npm package, no `./setup` script.**

This was decided after briefly building an `npx` installer (pure-Node launcher that cloned + scaffolded + collected keys + launched Claude) and then throwing it away. Why we reverted:

- **The reference projects don't use npm.** Garry Tan's `gstack` (66K★) and `gbrain` — the closest precedent (Claude-Code-native OSS) — onboard with `git clone … && ./setup` and "have your agent do it for you," explicitly *not* npx. Following that precedent over inventing our own.
- **No Node dependency on a Python tool.** `npx` forces every non-technical founder to have Node installed just to set up a Python project. `git clone` needs only git.
- **One less artifact to own.** An npm package is a second thing to version, publish, and keep in sync with the repo. CLAUDE.md already *is* the wizard (patterns #19, #20) — the installer largely duplicated it.
- **It barely moved the friction.** Pasting `npx create-gtm-engine` vs pasting `git clone …` is the same effort for a non-technical user; clone just has fewer prerequisites.

What this means for future work:
- **Don't rebuild an installer / setup wizard / npm package.** If onboarding feels rough, fix it *in CLAUDE.md* (the "When a user opens this repo" section), not by adding a new entry-point artifact. This is the third time the "build a setup tool" idea has come up and been rejected — see [[project_published]].
- **Keys are lazy (just-in-time), not an upfront setup step.** First-time setup is the *context interview only* — it asks for no keys. Each key is requested at the moment a step needs it (Anthropic when a workflow run starts, scrapers when that scraper runs), mentioning only the keys the chosen workflow uses. Reason: don't make a non-technical user hunt for keys before they've even said what they do. Keys still go *into the `.env` file, not the chat* (paste after the `=`; Claude confirms by reading the file, never echoes a value). The headless framing is planted for the future hybrid: unattended/auto/cron/big-batch runs are flagged as the clearest case for "you need your own Anthropic key." (See the lazy-keys decision below and [[reference_anthropic_auth]].)
- **The questions live once**, in `context.md.example`, asked by Claude — never duplicate them into a script.
- **CLAUDE.md assumes a fresh clone**: nothing pre-filled, `.env`/`context.md` may not exist. The "How they got here" note at the top of the user section says so.

**The setup gate — context (only) before any real work:** whatever the user's first message is, if `context.md` isn't filled, run the business interview BEFORE acting on it, then resume their original request. A workflow or outreach draft against empty `context.md` produces generic garbage (cold email that doesn't know the product, leads scored against no ICP) — worse than nothing. **The gate is keyed on context-completeness only — keys are NOT part of it** (keys are lazy, collected at run time; see above). The gate also doesn't fire on "is this the first message": a returning, already-configured user skips straight to their request (don't re-interview them every session). Only the Tour and plain "how does this work" questions are allowed with empty context. Codified at the top of CLAUDE.md's user section ("Always set up context first") and reinforced in Greeting + Running a workflow.

**The interactive-keyless / headless-keyed hybrid (chosen direction, partially shipped):** the goal is interactive use that needs no Anthropic key (Claude Code session = the brain, gstack-style) with a Python+API fallback for headless/auto/cron runs. Hard rule: *whoever makes the LLM call needs the credential, not whoever launched it* — so keyless-interactive specifically requires the reasoning to run in-session (the `workflow_ops.py` "Claude-as-brain" pattern), which exists for `linkedin_outreach` only (Sheets-only). Extending it to all 6 workflows is a real refactor (shared prompt files + two thin drivers: Python-API for headless, Claude-session for interactive). Caveat: big batches in-session are slow, eat the user's Claude subscription usage limits, and can drift/hit context limits — so even interactively, large runs may belong on the API path. **Decided 2026-06-17 to ship only the "lazy-key UX" first** (keys requested just-in-time, headless framing planted) and defer the in-session driver build. gstack confirmed (README) as the working precedent: pure in-session skills, no API key, `git clone … && ./setup`. See [[reference_anthropic_auth]].

**The auto-welcome trick (and why it's not a hook):** the goal was "a welcome message appears on its own the moment they open the folder." Verified against the Claude Code docs: **Claude Code never emits an assistant message at session start — it always waits for user input.** A `SessionStart` hook can't deliver a visible banner either: its stdout is injected as hidden context, not shown to the user (only stderr shows, and only on exit code 2, which reads as an error). So **do not add a SessionStart hook for the welcome — it cannot work.** The mechanism that *does* work: **seed the first prompt in the launch command** — the documented command is `claude "help me get started"`, which opens an interactive session and auto-submits that message, firing CLAUDE.md's greeting with zero typing. CLAUDE.md's greeting therefore leads with an explicit "Welcome to GTM Engine 👋" line, because it's the first thing the user sees. Bare `claude` still works — the greeting also triggers on "hi" / "what is this" / a sensed first-timer.
