...
**Brain backend (`GTM_BRAIN`) — run the "thinking" on a subscription instead of the API key.** Every workflow's LLM work (scoring, classification, copywriting, small-talk extraction) goes through one client built by `make_brain_client()` in `config.py`. `GTM_BRAIN` selects the backend: `api` (default) → `anthropic.Anthropic()`, billed to `ANTHROPIC_API_KEY`; `claude` → shells out to `claude -p` (subscription, no API key); `codex` → `codex exec` (ChatGPT plan). The CLI backends return a duck-typed stand-in exposing `resp.content[0].text`, so the ~35 `client.messages.create(...)` call sites are unchanged — only the 8 construction sites now call `make_brain_client()` (the 6 workflows + `scrapers/small_talk_scraper` + `skills/_copy_core.py`; `_common.detect_columns` receives its client as a param). Example: `GTM_BRAIN=claude python3 -m workflows.email_outreach.workflow …`. **Scrapers/search (Apify/Apollo/Exa/Parallel/Firecrawl) still use their own paid keys** — `GTM_BRAIN` only moves the Anthropic calls. One-time setup for `claude`: run `claude setup-token` and confirm `claude auth status` shows an OAuth/subscription method, not `api_key` (an env `ANTHROPIC_API_KEY` overrides the subscription, so the CLI backend strips it from each call's environment). Known limits of the CLI path: no `temperature`/`max_tokens` control (slightly less deterministic than the API's `temperature=0`), no prompt caching, and one CLI process per call (slower, draws down the plan's usage window) — fine at consulting-batch scale, not thousands/night. Both CLI backends run a **preflight** in `make_brain_client()` that fails fast with an actionable message instead of crashing mid-run: `claude` hard-stops if `claude auth status` shows `authMethod: api_key` (it would bill the API, not the subscription — fix with `claude setup-token` + remove `ANTHROPIC_API_KEY` from `~/.claude/settings.json` and your shell profile); `codex` hard-stops if the codex binary can't run. **Note: `codex exec` is SIGKILLed (-9) when spawned nested inside another Codex agent session or a supervised shell — codex sandboxes itself and the OS kills the nested process before it runs. This is not a gtm-engine bug and can't be fixed in Python; run the workflow from a plain terminal, or use `GTM_BRAIN=claude`.** `_run_codex_cli` captures the answer via `--output-last-message` (not stdout, which carries reasoning/tool framing); override the base command with `GTM_CODEX_CMD` if the installed version's flags differ. **Model id: the `model=` passed at each call site is an Anthropic id (`CLAUDE_MODEL`) that codex does not recognize — forwarding it as `-m` makes `codex exec` exit 1 ("model metadata not found"). So the codex backend IGNORES it and lets codex use your ChatGPT plan's default model; pin a specific OpenAI model with `GTM_CODEX_MODEL` if you want one.** (The `claude` backend still forwards `model` as-is, since those ARE Claude ids.)
...
- **Search cost — Exa summary is opt-out per call.** `search_web(...)` requests Exa `highlights` on every result plus a per-result LLM `summary`; that summary is billed per result on top of the search. Callers that only read result URLs/titles should pass `include_summary=False` — highlights still populate, the summary is dropped, cutting the content charge. Already off for the pure URL lookups in `competitor_analysis/steps.py` (`find_official_site`, LinkedIn-company, G2). Sites that read `r["summary"]` (enrichment, small-talk, text-scan fallbacks) keep it on. Note: `SEARCH_PROVIDER=auto` with **both** `EXA_API_KEY` and `PARALLEL_API_KEY` set resolves to *Exa-primary fallback*, so all traffic hits (and bills) Exa — set `SEARCH_PROVIDER=parallel` to route off Exa entirely.
- **Checkpoint location — persist across fresh workspaces.** Checkpoints default to `./.cache/checkpoints` (relative to cwd). A fresh Conductor workspace (new worktree/container per run) starts with an empty `.cache/`, so every re-run re-pays for searches the last run finished. Set `GTM_CHECKPOINT_DIR` (in `_common.py`) to a stable path that outlives the workspace (shared/mounted dir or an absolute `$HOME` path) so checkpoints survive across runs.
